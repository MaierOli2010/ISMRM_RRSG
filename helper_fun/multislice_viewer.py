...
def imshow(volume, vmin=None, vmax=None):
    if volume.ndim <= 3:
        ...
        ax.axis('off')
        plt.title("Slice "+str(volume.shape[0] // 2))

    elif volume.ndim == 4:
        fig, ax = plt.subplots(int(np.ceil(np.sqrt(volume.shape[0]))),
                               int(np.ceil(np.sqrt(volume.shape[0]))))
        fig.tight_layout()
    ...
    for i in range(len(ax)):
        ...
        if int((ax[i].index - event.step) >= volume.shape[0] or
           int((ax[i].index - event.step) < 0)):
            ...
            plt.title("Slice "+str(ax[i].index))
            fig.canvas.draw()
